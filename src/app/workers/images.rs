fn spawn_icon_worker(
    ...
    runtime_services.clone().spawn(async move {
        while let Some(request) = rx.recv().await {
            ...
            let Some(image) = load_icon_color_image(bytes) else {
                continue;
            };
            let _ = tx.send(IconResult {
                    ...
                    Ok(bytes) => {
                        ...
                        let job = handle.spawn_blocking(move || -> Result<GifAnimation> {
                            ...
                            for (frame_num, frame_result) in frames_iter.enumerate() {
                                ...
                                let frame_bytes = (width as u64)
                                    .saturating_mul(height as u64)
                                    .saturating_mul(4);
                                if !frames.is_empty()
                                    && total_rgba_bytes.saturating_add(frame_bytes)
                                        > GIF_ANIMATION_MAX_TOTAL_RGBA_BYTES
                                {
                                    break;
                                }
                                let color_image = egui::ColorImage::from_rgba_unmultiplied(
...
fn spawn_local_mod_image_worker(
    runtime_services: &RuntimeServices,
    portable: PortablePaths,
    global_generation: Arc<AtomicU64>,
    cache_limit_bytes: Arc<AtomicU64>,
    rx: WorkerRx<LocalModImageRequest>,
    tx: WorkerTx<LocalModImageResult>,
) {
    ...
    let thumbnail_byte_cache = runtime_services.thumbnail_byte_cache.clone();
    for _ in 0..LOCAL_MOD_IMAGE_WORKERS {
        let rx = Arc::clone(&rx);
        let tx = tx.clone();
        ...
        let global_generation = Arc::clone(&global_generation);
        let full_limiter = Arc::clone(&full_limiter);
        let full_decode_limiter = Arc::clone(&full_decode_limiter);
        let thumbnail_byte_cache = thumbnail_byte_cache.clone();
        runtime_services.spawn(async move {
            loop {
                ...
                if request.generation < global_generation.load(Ordering::Relaxed) {
                ...
                match request.payload {
                    LocalModImagePayload::Path {
                        path,
                        thumb_profile,
                    } => {
                        let path_for_meta = path.clone();
                        let (mtime, size) = handle
                            .spawn_blocking(move || {
                                fs::metadata(&path_for_meta).ok().map(|m| {
                                    let mt = m
                                        .modified()
                                        .ok()
                        ...
                        let cache_key = format!(
                            "local_thumb:{}:{}:{}:{}",
                            thumb_profile.suffix(),
                            mtime,
                            size,
                            hash64_hex(path_str.as_bytes())
                        );
                        ...
                        if matches!(
                            ...
                            if let Some(cached_bytes) = thumbnail_byte_cache.get(&cache_key) {
                                if let Some(image_thumb) =
                                    load_cover_color_image(cached_bytes.as_slice())
                                {
                                    let _ = tx.send(LocalModImageResult {
                            ...
                            if let Ok(Ok(Some(cached_bytes))) = handle
                                .spawn_blocking(move || {
                                ...
                                .await
                            {
                                if let Some(image_thumb) = load_cover_color_image(&cached_bytes) {
                                    thumbnail_byte_cache.insert(cache_key.clone(), cached_bytes);
                                    let _ = tx.send(LocalModImageResult {
                                        texture_key: texture_key.clone(),
                                        image_full: None,
                                        image_thumb: Some(image_thumb),
                        ...
                        match request.mode {
                            LocalModImageMode::CardThumbOnly | LocalModImageMode::ThumbOnly => {
                                let _thumb_permit = thumb_limiter.acquire().await.ok();
                                let decoded = handle
                                    .spawn_blocking(move || {
                                        let bytes = std::fs::read(&path_for_io).ok()?;
                                        resize_image_bytes_to_thumbnail_rgba(&bytes, thumb_profile)
                                    })
                                    .await
                                    .ok()
                                    .flatten();
                                if let Some((w, h, rgba)) = decoded {
                                    ...
                                    if let Ok(png_bytes) = resize_rgba_to_png(w, h, rgba) {
                                        thumbnail_byte_cache
                                            .insert(cache_key.clone(), png_bytes.clone());
                            ...
                            LocalModImageMode::FullOnly => {
                                ...
                                let portable_for_full_get = portable.clone();
                                let full_cache_key_for_get = full_cache_key.clone();
                                if let Ok(Ok(Some(cached_bytes))) = handle
                                    .spawn_blocking(move || {
                                        persistence::cache_get(
                                            &portable_for_full_get,
                                            &full_cache_key_for_get,
                                        )
                                    })
                                    .await
                                {
                                    if let Some(image_full) = load_cover_color_image(&cached_bytes)
                                    {
                                        let _ = tx.send(LocalModImageResult {
                                            texture_key: texture_key.clone(),
                                            image_full: Some(image_full),
                                            image_thumb: None,
                                            done: true,
                                            thumb_generated: false,
                                            thumb_meta: None,
                                        ...
                                        let _ =
                                            persistence::cache_remove(&portable, &full_cache_key);
                                    }
                                };
                                let _full_permit = full_limiter.acquire().await.ok();
                                let _decode_permit = full_decode_limiter.acquire().await.ok();
                                let full = handle
                                    .spawn_blocking(move || {
                                        let bytes = std::fs::read(&path_for_io).ok()?;
                                    ...
                                    .ok()
                                    .flatten();
                                let mut image_thumb = None;
                                if let Some(image_full) = full.as_ref() {
                                    let w = image_full.width() as u32;
                                    let h = image_full.height() as u32;
                                    let rgba: Vec<u8> = image_full
                                        .pixels
                                        .iter()
                    ...
                    LocalModImagePayload::CardThumb {
                        mod_root,
                        source_path,
                        source_url,
                        expected_meta,
                        ...
                        let thumb_path =
                            HestiaApp::resolve_mod_thumb_path(&mod_root, ThumbnailProfile::Card);
                        let thumb_ram_key = format!("card-thumb:{texture_key}");
                        ...
                        if thumb_bytes.is_none() && !force_regen && thumb_path.exists() {
                            if let Some(bytes) = tokio::fs::read(&thumb_path).await.ok() {
                                if load_cover_color_image(&bytes).is_some() {
                                    thumbnail_byte_cache
                                        .insert(thumb_ram_key.clone(), bytes.clone());
...
fn spawn_cover_worker(
    runtime_services: &RuntimeServices,
    mut rx: WorkerRx<CoverRequest>,
    tx: WorkerTx<CoverResult>,
) {
    runtime_services.spawn(async move {
        while let Some(request) = rx.recv().await {
            let Some(bytes) = game_cover_bytes(&request.game_id) else {
                continue;
            };
            let Some(image) = load_cover_color_image(bytes) else {
                continue;
            };
...
fn load_cover_color_image_thumbnail(
    ...
    resize_image_bytes_to_thumbnail_rgba(bytes, profile).and_then(|(w, h, rgba)| {
...
fn resize_image_bytes_to_thumbnail_rgba(
    ...
    resize_rgba_to_thumbnail_rgba(src_w, src_h, raw, profile)
}
fn resize_rgba_to_thumbnail_rgba(
    src_w: u32,
    src_h: u32,
    rgba: Vec<u8>,
    profile: ThumbnailProfile,
...
fn resize_rgba_to_fit_rgba_with_options(
    ...
    let scale = (target_w as f32 / src_w as f32)
        .min(target_h as f32 / src_h as f32)
        .min(1.0);
    ...
    if success {
        Some((dst_w, dst_h, dst_image.into_vec()))
    } else {
        None
    }
}
fn load_title_icon_texture(
    ...
    let image = load_icon_color_image(bytes)?;
    Some(ctx.load_texture(texture_name, image, egui::TextureOptions::LINEAR))
}
fn load_icon_color_image(bytes: &[u8]) -> Option<egui::ColorImage> {
    let image = image::load_from_memory(bytes).ok()?.to_rgba8();
    let resized = image::imageops::resize(
        &image,
        GAME_ICON_TEXTURE_SIZE,
        GAME_ICON_TEXTURE_SIZE,
